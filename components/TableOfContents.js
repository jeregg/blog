...
export default function TableOfContents ({ blockMap, className, style }) {
  ...
  return (
    <aside
      className={cn(className, 'pl-4 text-sm text-gray-400 dark:text-neutral-400')}
      style={style}
    >
      {nodes.map(node => (
        <div key={node.id}>
          <a
            ...
            className="block py-1 hover:text-gray-800 dark:hover:text-white cursor-pointer transition duration-100"
