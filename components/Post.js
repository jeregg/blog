...
export default function Post (props) {
  ...
  return (
    <article className={cn('flex flex-col', fullWidth ? 'md:px-24' : 'items-center')}>
      <h1 className={cn(
        'w-full uppercase font-extrabold text-3xl text-gray-800 dark:text-white',
      ...
      {post.type[0] !== 'Page' && (
        <nav className={cn(
          'w-full flex mt-3.5 items-start uppercase text-sm font-bold text-gray-800 dark:text-gray-400',
          { 'max-w-3xl px-4': !fullWidth }
        )}>
      ...
      <div className="self-stretch -mt-4 flex flex-col items-center">
        ...
        <div className={cn('order-first w-full max-w-3xl', fullWidth ? 'flex-none' : 'flex-1')}>
